...
def collate_train(data):
    ...
    clip_video_features, frame_video_features, captions, idxs, cap_ids, video_ids, sc_mask_t, sc_mask_v = zip(*data)
    ...
    for index, caps in enumerate(sc_mask_t):
        sc_masks_t.extend(cap for cap in caps)
    for index, caps in enumerate(sc_mask_v):
        sc_masks_v.extend(cap for cap in caps)

    target = torch.zeros(len(all_lengths), 60, feat_dim)
    words_mask = torch.zeros(len(all_lengths), 60)
    ...
    return dict(clip_video_features=clip_videos,
                ...
                text_labels=labels,
                sc_masks_t=sc_masks_t,
                sc_masks_v=sc_masks_v,
                )
...
def collate_text_val(data):
    ...
    captions,idxs, cap_ids, sc_mask_t, sc_mask_v = zip(*data)

    if captions[0] is not None:
        lengths = [len(cap) for cap in captions]

        target = torch.zeros(len(captions), 60, captions[0].shape[-1])
        words_mask = torch.zeros(len(captions), 60)
    ...
    return target, words_mask, idxs, cap_ids, sc_mask_t, sc_mask_v
